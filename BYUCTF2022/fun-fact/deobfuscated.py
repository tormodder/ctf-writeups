...
def option_three():
    ...
    #Note on the randomness here: The encrypted string is always the same, regardless of if the program is restarted
    ...
    #For every letter x in user_input
    #binary XOR is called on unicode x and unicode key, The result of which is decoded into a char
    # The result is joined together in the encrypted string
    encrypted = "".join([chr(ord(x) ^ ord(key)) for x in user_input])
    print("encrypted: ", encrypted)

    #The phrase which, when encrypted, gives this string is the flag
